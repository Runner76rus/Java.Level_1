...
public class Homework {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < equalArr.length; i++) equalArr[i] = (int) (Math.random() * 3);

        changeValueArray(arrBin);
        System.out.println();
        fillArray();
        System.out.println();
        multiplyArray(casArr);
        System.out.println();
        fillDiagonal(squareArr);
        System.out.println();
        getExtremeValue();
        System.out.println();
        System.out.println(isEqualArray(equalArr));
        System.out.println();
        shiftValueArray(equalArr, n);

    }

    public static void changeValueArray(byte[] arrBin) {
        System.out.println(Arrays.toString(arrBin));
        for (int i = 0; i < arrBin.length; i++) {
            if (arrBin[i] == 0) arrBin[i] = 1;
            else arrBin[i] = 0;
        }
        System.out.println(Arrays.toString(arrBin));
    }
    ...
    public static void getExtremeValue() {
        ...
        int i;
        for (i = 0; i < extArr.length; i++) extArr[i] = (int) (Math.random() * 25);
    ...
    public static boolean isEqualArray(int[] equalArr) {
    ...
    public static void shiftValueArray(int[] equalArr, int n) {
        int i = 0;
        ...
        if (n > 0) {
            for (int v : cloneArr) {
                ...
                if (delta < 0) delta = 0;
                if (i + n < equalArr.length) equalArr[i + n] = v;
                else equalArr[delta] = v;
                i++;
            }
        } else {
            for (int j = equalArr.length - 1; j >= 0; j--) {
                if (j + n >= 0) equalArr[j + n] = cloneArr[j];
                else equalArr[equalArr.length + j + n] = cloneArr[j];
            }
        }
        System.out.println(Arrays.toString(equalArr));
    }
